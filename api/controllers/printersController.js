...
export const createPrinter = async (req, res, next) => {
  ...
  try {
    const printer = await Printer.findOne({ modelName: printerToCreate.modelName, manufacturer: printerToCreate.manufacturer });
    if (printer) {
    ...
    try {
      ...
      if (!printer) {
        res.status(404).send();
          ...
          const updatedPrinter = await Printer.findOneAndUpdate({ _id: id }, printerToUpdate);
          res.status(200).json({
